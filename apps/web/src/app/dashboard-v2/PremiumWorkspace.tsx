...
function PremiumModulePage({ view }: { view: Exclude<PremiumView, "dashboard"> }) {
  ...
  const featuredRows = view === "customers"
    ? [["Meridian Studio GmbH", "4 offene Dokumente", "2.467,00 EUR"], ["Aurora Labs GmbH", "Zahlung erhalten", "719,05 EUR"], ["Pixel Perfect Ltd.", "Neues Projekt", "Design Sprint"]]
    : view === "projects"
      ? [["Website Redesign", "Phase 2 aktiv", "78%"], ["Brand Portal", "Review offen", "42%"], ["DATEV Export", "Bereit", "100%"]]
      : view === "invoices" || view === "offers"
        ? invoices.map(([number, customer, status, amount]) => [number, customer, `${status} · ${amount}`])
        : [["Premium Check", "Heute", "Aktiv"], ["Automatische Erinnerung", "Morgen", "Geplant"], ["Monatsreport", "Freitag", "Bereit"]]
    ...
    <section className={styles.modulePage}>
      ...
      <section className={styles.moduleGrid}>
        <article className={`${styles.panel} ${styles.moduleCard}`}>
          <div className={styles.panelHead}><h2>Status</h2><span>Live Vorschau</span></div>
          <div className={styles.moduleStats}>
            <div><strong>24</strong><span>Eintraege</span></div>
            <div><strong>8</strong><span>Heute aktiv</span></div>
            <div><strong>96%</strong><span>Qualitaet</span></div>
          </div>
        </article>
        ...
        <article className={`${styles.panel} ${styles.moduleCard}`}>
          ...
          <div className={styles.actionStrip}>
            <button type="button"><Plus size={16} />Neu</button>
            <button type="button"><Search size={16} />Suchen</button>
            <button type="button"><BarChart3 size={16} />Analyse</button>
          </div>
        </article>
        ...
        <article className={`${styles.panel} ${styles.moduleTable}`}>
          ...
          <div className={styles.pipelineList}>
            {featuredRows.map(([title, subtitle, value]) => (
              <div key={`${title}-${value}`}>
