...
export default async function PremiumNotificationsPage({ searchParams }: DashboardV2SearchPageProps) {
  const { query, theme } = await dashboardV2ParamsFromSearchParams(searchParams)

  return <PremiumWorkspacePage view="notifications" initialSearchQuery={query} initialTheme={theme} />
}
