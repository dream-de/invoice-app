...
function marketplaceStatusClass(status: ResolvedMarketplaceModule["runtimeStatus"]) {
  if (status === "Aktiv") return "bg-emerald-50 text-emerald-700 ring-emerald-200"
  if (status === "Installiert") return "bg-sky-50 text-sky-700 ring-sky-200"
  if (status === "Verfuegbar") return "bg-slate-100 text-slate-700 ring-slate-200"
  return "bg-zinc-100 text-zinc-600 ring-zinc-200"
}

function MarketplaceStatusPill({ status }: { status: ResolvedMarketplaceModule["runtimeStatus"] }) {
  return (
    <span className={`inline-flex whitespace-nowrap rounded-md px-2 py-1 text-[11px] font-extrabold ring-1 ${marketplaceStatusClass(status)}`}>
      {status}
    </span>
  )
}

function MarketplaceIcon({ category }: { category: MarketplaceModuleCategory }) {
  const iconClass = "h-4 w-4"
  if (category === "Finanzen") return <Landmark className={iconClass} />
  if (category === "KI") return <Eye className={iconClass} />
  if (category === "E-Commerce") return <Store className={iconClass} />
  if (category === "Projektmanagement") return <Workflow className={iconClass} />
  if (category === "Produktion") return <Archive className={iconClass} />
...
function LicenseBillingSettingsPage() {
  ...
  const [installedMarketplaceKeys, setInstalledMarketplaceKeys] = useState<MarketplaceModuleKey[]>(() => installedExtensions.map((extension) => extension.marketplaceKey))
  const [activeMarketplaceKeys, setActiveMarketplaceKeys] = useState<MarketplaceModuleKey[]>(() => installedExtensions.filter((extension) => extension.status === "Aktiv").map((extension) => extension.marketplaceKey))
  const [selectedMarketplaceKey, setSelectedMarketplaceKey] = useState<MarketplaceModuleKey | null>(null)
  const marketplaceModules = resolveMarketplaceModules({
    installedExtensionKeys: installedMarketplaceKeys,
    activeExtensionKeys: activeMarketplaceKeys
  })
  const activeFeatureFlags = marketplaceModules
    .filter((module) => module.active && module.canonicalFeatureFlag)
    .map((module) => module.canonicalFeatureFlag!)
  const activeInstalledExtensions = marketplaceModules.filter((module) => module.installed)
  const visiblePremiumModules = getVisiblePremiumModules({
    installedExtensionKeys: installedMarketplaceKeys,
    activeExtensionKeys: activeMarketplaceKeys
  })
  const selectedMarketplaceModule = marketplaceModules.find((module) => module.key === selectedMarketplaceKey) ?? marketplaceModules[0]
  ...
  function renderOverview() {
    const cards = [
      ...
      ["Installierte Erweiterungen", String(activeInstalledExtensions.length), "Aus zentraler Marketplace-Definition"],
      ["Dynamische Module", String(visiblePremiumModules.length), "Sichtbar nach Feature Flags"],
  ...
  function installMarketplaceModule(key: MarketplaceModuleKey) {
    setInstalledMarketplaceKeys((current) => current.includes(key) ? current : [...current, key])
    setSelectedMarketplaceKey(key)
  }

  function activateMarketplaceModule(key: MarketplaceModuleKey) {
    setInstalledMarketplaceKeys((current) => current.includes(key) ? current : [...current, key])
    setActiveMarketplaceKeys((current) => current.includes(key) ? current : [...current, key])
    setSelectedMarketplaceKey(key)
  }

  function deactivateMarketplaceModule(key: MarketplaceModuleKey) {
    setActiveMarketplaceKeys((current) => current.filter((item) => item !== key))
    setSelectedMarketplaceKey(key)
  }

  function renderMarketplaceActions(module: ResolvedMarketplaceModule) {
    if (module.runtimeStatus === "Nicht verfuegbar") {
      return <button type="button" disabled className="rounded-md border border-[var(--settings-line)] px-3 py-2 text-xs font-black text-[var(--settings-muted)]">Nicht verfuegbar</button>
    }

    if (module.runtimeStatus === "Aktiv") {
      return <button type="button" onClick={() => deactivateMarketplaceModule(module.key)} className="rounded-md border border-[var(--settings-line)] px-3 py-2 text-xs font-black text-[var(--settings-title)]">Deaktivieren</button>
    }

    if (module.runtimeStatus === "Installiert") {
      return <button type="button" onClick={() => activateMarketplaceModule(module.key)} className="rounded-md bg-[var(--settings-accent-strong)] px-3 py-2 text-xs font-black text-white">Aktivieren</button>
    }

    return <button type="button" onClick={() => installMarketplaceModule(module.key)} className="rounded-md bg-[var(--settings-accent-strong)] px-3 py-2 text-xs font-black text-white">Installieren</button>
  }

  function renderExtensions() {
    return (
      <div className="grid gap-3 md:grid-cols-2">
        {activeInstalledExtensions.map((extension) => (
          <div key={extension.key} className="rounded-lg border border-[var(--settings-line)] bg-[var(--settings-surface)] p-4 shadow-[var(--settings-card-shadow)]">
            <div className="flex items-start justify-between gap-3">
              <div className="flex min-w-0 gap-3">
                <span className="inline-flex h-9 w-9 shrink-0 items-center justify-center rounded-md bg-[var(--settings-accent-soft)] text-[var(--settings-title)]"><MarketplaceIcon category={extension.category} /></span>
                <div className="min-w-0">
                  <h3 className="text-base font-black text-[var(--settings-title)]">{extension.name}</h3>
                  <p className="mt-1 text-xs font-bold text-[var(--settings-muted)]">{extension.featureFlag ?? "Provider-Konfiguration"}</p>
                  <p className="mt-1 text-xs font-semibold text-[var(--settings-muted)]">{extension.description}</p>
                </div>
              </div>
              <MarketplaceStatusPill status={extension.runtimeStatus} />
            </div>
            <div className="mt-4 flex flex-wrap gap-2">
              {renderMarketplaceActions(extension)}
              <button type="button" onClick={() => setSelectedMarketplaceKey(extension.key)} className="rounded-md border border-[var(--settings-line)] px-3 py-2 text-xs font-black text-[var(--settings-title)]">Details</button>
            </div>
          </div>
        ))}
      </div>
  ...
  function renderMarketplace() {
    const groupedModules = marketplaceModulesByCategory.map((group) => ({
      category: group.category,
      modules: marketplaceModules.filter((module) => module.category === group.category)
    }))

    return (
      <div className="grid gap-4">
        <div className="rounded-lg border border-[var(--settings-line)] bg-[var(--settings-surface)] p-4 shadow-[var(--settings-card-shadow)]">
          <div className="flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
            <div>
              <p className="text-[11px] font-extrabold uppercase text-[var(--settings-muted)]">Details</p>
              <h3 className="text-lg font-black text-[var(--settings-title)]">{selectedMarketplaceModule.name}</h3>
              <p className="mt-1 max-w-3xl text-sm font-bold text-[var(--settings-muted)]">{selectedMarketplaceModule.detail}</p>
            </div>
            <MarketplaceStatusPill status={selectedMarketplaceModule.runtimeStatus} />
          </div>
        </div>
        ...
        {groupedModules.map((group) => (
          <section key={group.category} className="grid gap-3">
            <div className="flex items-center gap-2">
              <span className="inline-flex h-8 w-8 items-center justify-center rounded-md bg-[var(--settings-accent-soft)] text-[var(--settings-title)]"><MarketplaceIcon category={group.category} /></span>
              <div>
                <p className="text-[11px] font-extrabold uppercase text-[var(--settings-muted)]">Kategorie</p>
                <h3 className="font-black text-[var(--settings-title)]">{group.category}</h3>
              </div>
            </div>
            <div className="grid gap-3 md:grid-cols-2 xl:grid-cols-3">
              {group.modules.map((module) => (
                <article key={module.key} className="rounded-lg border border-[var(--settings-line)] bg-[var(--settings-surface)] p-4 shadow-[var(--settings-card-shadow)]">
                  <div className="flex items-start justify-between gap-3">
                    <div className="flex min-w-0 gap-3">
                      <span className="inline-flex h-9 w-9 shrink-0 items-center justify-center rounded-md bg-[var(--settings-accent-soft)] text-[var(--settings-title)]"><MarketplaceIcon category={module.category} /></span>
                      <div className="min-w-0">
                        <h4 className="font-black text-[var(--settings-title)]">{module.name}</h4>
                        <p className="mt-1 text-xs font-bold text-[var(--settings-muted)]">{module.description}</p>
                      </div>
                    </div>
                    <MarketplaceStatusPill status={module.runtimeStatus} />
                  </div>
                  <div className="mt-3 flex flex-wrap gap-1.5">
                    <span className="rounded-md bg-[var(--settings-subtle)] px-2 py-1 text-[11px] font-extrabold text-[var(--settings-muted)]">{module.featureFlag ?? "Provider"}</span>
                    <span className="rounded-md bg-[var(--settings-subtle)] px-2 py-1 text-[11px] font-extrabold text-[var(--settings-muted)]">{module.recommendedPlan}</span>
                  </div>
                  <div className="mt-4 flex flex-wrap gap-2">
                    {renderMarketplaceActions(module)}
                    <button type="button" onClick={() => setSelectedMarketplaceKey(module.key)} className="rounded-md border border-[var(--settings-line)] px-3 py-2 text-xs font-black text-[var(--settings-title)]">Details</button>
                  </div>
                </article>
              ))}
            </div>
