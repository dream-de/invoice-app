...
export default async function PremiumLogsPage({ searchParams }: DashboardV2SearchPageProps) {
  const { query, theme } = await dashboardV2ParamsFromSearchParams(searchParams)

  return <PremiumWorkspacePage view="logs" initialSearchQuery={query} initialTheme={theme} />
}
