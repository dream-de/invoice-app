...
async function getInvoices(request: Request) {
  ...
  const rows = await prisma.$queryRawUnsafe('SELECT "id", "number", "type", "status", "customerId", "projectId", "grossTotal", "createdAt" FROM "Invoice" WHERE ("companyId" = $2 OR $2 IS NULL) ORDER BY "createdAt" DESC LIMIT 100', key.tenantId, key.companyId)
  await prisma.$executeRawUnsafe('UPDATE "ApiKey" SET "lastUsedAt" = CURRENT_TIMESTAMP WHERE "keyHash" = $1', hash)
  return NextResponse.json({ ok: true, entity: "invoices", data: rows })
}

export const GET = withApiAuditLog(getInvoices, {
  module: "invoices",
  title: "API Rechnungen abgerufen",
  description: "Externe API hat Rechnungen abgerufen."
})
