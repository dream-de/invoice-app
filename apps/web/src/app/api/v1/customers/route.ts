...
async function getCustomers(request: Request) {
  ...
  const rows = await prisma.$queryRawUnsafe('SELECT "id", "number", "name", "email", "status", "createdAt" FROM "Customer" WHERE ("companyId" = $2 OR $2 IS NULL) ORDER BY "createdAt" DESC LIMIT 100', key.tenantId, key.companyId)
  await prisma.$executeRawUnsafe('UPDATE "ApiKey" SET "lastUsedAt" = CURRENT_TIMESTAMP WHERE "keyHash" = $1', hash)
  return NextResponse.json({ ok: true, entity: "customers", data: rows })
}

export const GET = withApiAuditLog(getCustomers, {
  module: "customers",
  title: "API Kunden abgerufen",
  description: "Externe API hat Kunden abgerufen."
})
