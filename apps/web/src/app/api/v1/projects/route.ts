...
async function getProjects(request: Request) {
  ...
  const rows = await prisma.$queryRawUnsafe('SELECT "id", "code", "name", "status", "customerId", "createdAt" FROM "Project" WHERE ("companyId" = $2 OR $2 IS NULL) ORDER BY "createdAt" DESC LIMIT 100', key.tenantId, key.companyId)
  await prisma.$executeRawUnsafe('UPDATE "ApiKey" SET "lastUsedAt" = CURRENT_TIMESTAMP WHERE "keyHash" = $1', hash)
  return NextResponse.json({ ok: true, entity: "projects", data: rows })
}

export const GET = withApiAuditLog(getProjects, {
  module: "projects",
  title: "API Projekte abgerufen",
  description: "Externe API hat Projekte abgerufen."
})
