...
import { FONT_OPTIONS, INVOICE_LAYER_NAMES, INVOICE_LAYER_Z, LAYER_TYPE_ICON, createDynamicTokenGroups, getElementTypeLabel } from "@/lib/document-templates/editor-options";
import { createTemplatePreviewInvoice } from "@/lib/document-templates/preview";
import { DEFAULT_TABLE_CONFIG, TEMPLATE_NEW_SCALE_MAX, TEMPLATE_NEW_SCALE_MIN, decreaseTemplateScale, deepClone, increaseTemplateScale, cloneTemplateElement, createTemplateElement, duplicateTemplateElements, moveTemplateLayer, moveTemplateLayerTo, removeTemplateElement, removeTemplateElements, getTemplateElementForClipboard, isEditableShortcutTarget, moveTemplateElementWithKeyboard, createTemplateSavePayload } from "@/lib/document-templates/editor-utils";
...
export default function NewInvoiceTemplatePage() {
  ...
  const { t } = useLanguage();
  const templateId = searchParams?.get("templateId") || `invoice-template-${Date.now()}`;
  ...
  const [template, setTemplate] = useState<DocumentTemplate>({
    ...
    id: "new-invoice-template",
    name: t("templates.editor.templateName.newInvoice"),
  });
            ...
            return (
              ...
              >
                ...
                <span className="min-w-0 flex-1">
                  <span className="block truncate text-sm font-black text-slate-700">{INVOICE_LAYER_NAMES[element.id] ?? element.id}</span>
        ...
        {selectedElement.type === "logo" && (
          <>
            ...
            <div>
              ...
              <ColorControl label="Farbe" value={selectedElement.backgroundColor ?? "#f4f4f4"} onChange={(value) => updateElement(selectedElement.id, { backgroundColor: value })} />
            </div>
          </>
        )}
          ...
          <>
            ...
            <div>
              ...
              <div className="grid grid-cols-[1fr_120px] gap-3">
                <ColorControl label="Farbe" value={selectedElement.color ?? selectedElement.backgroundColor ?? "#d1d5db"} onChange={(value) => updateElement(selectedElement.id, { color: value, backgroundColor: value })} />
  ...
  return (
    <div className="dark-template-editor h-screen overflow-hidden bg-[#07090d] text-white">
      {statusMsg && (
      ...
      <div className="grid h-screen grid-cols-[292px_minmax(0,1fr)_292px]">
        <aside className="template-editor-left relative flex h-screen flex-col overflow-hidden border-r border-[#222] bg-[#111111] px-5 py-6 text-white shadow-[18px_0_42px_rgba(0,0,0,0.20)]">
          ...
          <div className="shrink-0 border-t border-white/8 bg-[#0f0f0f] pt-5">
            <button type="button" onClick={exportTemplatePdf} className="w-full rounded-full bg-white px-4 py-3 text-xs font-black text-slate-950 shadow-[0_18px_45px_rgba(0,0,0,0.28)]">{t("templates.editor.left.pdfExport")}</button>
            <div className="mt-4 flex items-center justify-center gap-4 text-[11px] font-black text-white/40"><button type="button" aria-label={t("templates.editor.zoom.out")} title={t("templates.editor.zoom.out")} onClick={() => setScale((s) => decreaseTemplateScale(s, TEMPLATE_NEW_SCALE_MIN))}>−</button><span>{Math.round(scale * 100)}%</span><button type="button" aria-label={t("templates.editor.zoom.in")} title={t("templates.editor.zoom.in")} onClick={() => setScale((s) => increaseTemplateScale(s, TEMPLATE_NEW_SCALE_MAX))}>+</button></div>
          </div>
        </aside>
        <main className="template-editor-main relative h-screen overflow-auto bg-[#1c1c1c] px-10 py-9">
          <div className="flex min-h-[calc(100vh-72px)] w-full min-w-0 items-start justify-center">
            <DocumentCanvas template={template} invoice={preview} editable scale={scale} selectedId={selectedId} onSelectElement={(id) => toggleSelection(id || "", false)} onMoveElement={(id, x, y) => updateElement(id, { x: snap(x), y: snap(y) })} onResizeElement={(id, width, height) => updateElement(id, { width: snap(width), height: snap(height) })} onDeleteElement={deleteElement} selectedIds={selectedIds} onMoveMany={moveMany} guideX={guideX} guideY={guideY} showGrid={snapEnabled} gridSize={gridSize} showTemplateTokens showRuler={false} />
          </div>
        </main>
        <aside className="template-editor-right h-screen overflow-y-auto bg-white px-5 py-6 text-slate-950 shadow-[inset_1px_0_0_rgba(15,23,42,0.10)]">
