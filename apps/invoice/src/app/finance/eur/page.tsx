...
const transactions = [
  { date: "2025-02-17", source: "Bank", name: "Finanzamt Berlin", text: "Umsatzsteuervorauszahlung Jan 2025", amount: -990, type: "Ausgabe" },
  ...
  { date: "2025-02-03", source: "Bank", name: "StartUp Berlin AG", text: "Teilzahlung Strategieprojekt", amount: 1450, type: "Einnahme" },
  { date: "2025-01-22", source: "Bank", name: "Kunde Shop #1143", text: "PayPal Checkout", amount: 680, type: "Einnahme" },
  { date: "2025-01-19", source: "Bank", name: "Büro Center Berlin", text: "Büromaterial Q1", amount: -460, type: "Ausgabe" },
  { date: "2025-01-12", source: "Bank", name: "Telekom Deutschland", text: "Internet & Telefon Januar", amount: -189, type: "Ausgabe" },
  { date: "2025-01-08", source: "Bank", name: "Musterfirma GmbH", text: "Abschlagszahlung Website-Relaunch", amount: 2200, type: "Einnahme" }
]
      ...
      <section className="rounded-[34px] border border-[#e3e9f1] bg-[#f8f9fb] p-7 shadow-[0_8px_26px_rgba(15,23,42,0.05)]">
        <div className="mb-7 flex flex-col gap-5 xl:flex-row xl:items-center xl:justify-between">
          <div className="flex items-center gap-4">
            ...
            <div>
              <h1 className="text-3xl font-extrabold tracking-tight text-[#151b2b]">Anlage EUR</h1>
              <p className="mt-1 text-sm font-medium text-[#64748b]">
                Klassifizierung und Auswertung für Steuerjahr 2025.
              </p>
            </div>
          </div>
        ...
        <div className="grid gap-5 xl:grid-cols-[0.95fr_1.05fr]">
          <div className="rounded-[28px] border border-[#e1e7ef] bg-white p-5 shadow-sm">
            <div className="mb-4 flex items-center justify-between">
              <h2 className="text-lg font-extrabold text-[#151b2b]">Queue</h2>
              <span className="text-sm font-medium text-[#7b8799]">8 Einträge</span>
            </div>
            ...
            <div className="grid grid-cols-2 gap-2 rounded-[22px] border border-[#e6ebf1] bg-[#f7f9fc] p-2 text-center text-sm font-semibold text-[#64748b]">
              <button className="rounded-full bg-white px-4 py-2 text-[#151b2b] shadow-sm">Offen (8)</button>
              <button className="rounded-full px-4 py-2">Klassifiziert (0)</button>
              <button className="rounded-full px-4 py-2">Ausgeschl. (0)</button>
              <button className="rounded-full px-4 py-2">Alle (8)</button>
            </div>

            <div className="mt-4 flex items-center gap-3 rounded-full border border-[#e1e7ef] bg-white px-4 py-3">
              <Search className="h-4 w-4 text-slate-400" />
              <input className="w-full bg-transparent text-sm font-medium outline-none placeholder:text-slate-400" placeholder="Gegenpartei, Zweck oder Datum..." />
            </div>

            <div className="mt-4 grid grid-cols-2 gap-3">
              <button className="rounded-full border border-[#e1e7ef] bg-white px-4 py-2.5 text-sm font-semibold text-[#1f2937]">Alle Typen</button>
              <button className="rounded-full border border-[#e1e7ef] bg-white px-4 py-2.5 text-sm font-semibold text-[#1f2937]">Neueste zuerst</button>
            </div>
            ...
            <div className="mt-4 rounded-[22px] border border-[#e6ebf1] bg-[#f7f9fc] p-3">
              <div className="mb-3 flex items-center justify-between text-sm">
                ...
                <button className="font-medium text-[#64748b]">Auswahl löschen</button>
              </div>

              <div className="grid gap-2">
                <button className="rounded-full bg-white px-4 py-2.5 text-left text-sm font-medium text-[#94a3b8]">Vorschlag anwenden</button>
                <button className="rounded-full bg-white px-4 py-2.5 text-left text-sm font-medium text-[#94a3b8]">Als privat/Transfer markieren</button>
              ...
              {transactions.map((item) => (
                ...
                >
                  ...
                  <div className="mt-3 flex flex-wrap gap-2">
                    <span className="rounded-full bg-orange-50 px-3 py-1 text-xs font-bold text-orange-700">Offen</span>
                    <span className="rounded-full bg-[#eef2f7] px-3 py-1 text-xs font-bold text-[#475569]">{item.type}</span>
                    <span className="rounded-full bg-[#eef2f7] px-3 py-1 text-xs font-bold text-[#475569]">Stichwort</span>
                  </div>
                </button>
              ))}
            </div>
          ...
          <div className="space-y-5">
            <div className="rounded-[28px] border border-[#e1e7ef] bg-white p-5 shadow-sm">
              <h2 className="text-lg font-extrabold text-[#151b2b]">Klassifizierung</h2>
              ...
              <div className="mt-5 rounded-[24px] border border-dashed border-[#d8e0ea] bg-[#f7f9fc] p-8 text-center">
                <p className="text-base font-extrabold text-[#151b2b]">
                  {selected ? selected : "Kein Eintrag ausgewählt"}
                </p>
                <p className="mt-2 text-sm font-medium text-[#64748b]">
                  {selected ? "Kategorie auswählen und Klassifizierung speichern." : "Wählen Sie links einen Eintrag zur Klassifizierung aus."}
                </p>
              </div>
            </div>

            <div className="rounded-[28px] border border-[#e1e7ef] bg-white p-5 shadow-sm">
              <h2 className="text-lg font-extrabold text-[#151b2b]">Report</h2>
                ...
                {[
                  ...
                  <div key={label} className="rounded-[20px] bg-[#f7f9fc] p-4">
                    ...
                    <p className="mt-2 text-lg font-extrabold text-[#111827]">
                      {label === "Unklassifiziert" ? value : <Currency value={Number(value)} />}
                    </p>
                  </div>
                ))}
              </div>
