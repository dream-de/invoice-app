...
export default function ArticlesPage() {
  ...
  async function createArticle() {
    ...
    if (!response.ok || !result.ok) {
      alert(result.error || "Artikel konnte nicht gespeichert werden.")
      return
    }
  ...
  async function updateArticle() {
    ...
    if (!response.ok || !result.ok) {
      alert(result.error || "Artikel konnte nicht aktualisiert werden.")
      return
    }
  ...
  async function deleteArticle(article: ArticleItem) {
    const confirmed = window.confirm(`Artikel "${article.name}" wirklich löschen?`)
    if (!confirmed) return
    ...
    if (!response.ok || !result.ok) {
      alert(result.error || "Artikel konnte nicht gelöscht werden.")
      return
    }
  ...
  async function recognizeImportFile(file: File) {
    ...
    if (!response.ok || !result.ok) {
      alert(result.warnings?.[0] || result.error || "Artikel konnten nicht erkannt werden.")
      return
    }
  ...
  async function importArticles() {
    ...
    if (!response.ok || !result.ok) {
      alert(result.error || "Artikel konnten nicht gespeichert werden.")
      return
    }
      ...
      <section className="rounded-[34px] border border-[#e3e9f1] bg-[#f8f9fb] p-7 shadow-[0_8px_26px_rgba(15,23,42,0.05)]">
        <div className="flex flex-col gap-5 xl:flex-row xl:items-start xl:justify-between">
          <div>
            ...
            <p className="mt-3 text-base font-medium text-[#64748b]">
              {loading ? "Lade Artikel..." : `${items.length} Einträge`}
            </p>
          </div>
          ...
          <div className="flex flex-wrap items-center gap-3">
            ...
            <div className="inline-flex rounded-full bg-[#eceff3] p-1">
              <button onClick={() => setView("grid")} className={`flex h-9 w-10 items-center justify-center rounded-full transition ${view === "grid" ? "bg-white text-[#111827] shadow-sm" : "text-[#7b8799]"}`} aria-label="Kartenansicht">
                <Grid2X2 className="h-4 w-4" />
              </button>
              <button onClick={() => setView("list")} className={`flex h-9 w-10 items-center justify-center rounded-full transition ${view === "list" ? "bg-white text-[#111827] shadow-sm" : "text-[#7b8799]"}`} aria-label="Listenansicht">
            ...
            <button onClick={() => setPanelOpen(true)} className="flex h-12 w-12 items-center justify-center rounded-full bg-[var(--brand-lime)] text-black shadow-sm transition hover:brightness-95" aria-label="Neuer Artikel">
        ...
        <div className="mt-8 flex flex-col gap-4 xl:flex-row xl:items-center xl:justify-between">
          <div className="flex h-12 w-full max-w-[390px] items-center gap-3 rounded-full bg-white px-5 shadow-sm ring-1 ring-[#e1e7ef]">
            ...
            <input value={query} onChange={(event) => setQuery(event.target.value)} className="w-full bg-transparent text-sm font-semibold text-[#334155] outline-none placeholder:text-[#94a3b8]" placeholder="Artikel suchen" />
          </div>
            ...
            {filtered.map((article) => {
              ...
              return (
                <article key={article.id} className="rounded-[30px] border border-[#e5eaf0] bg-white p-6 shadow-sm transition hover:border-[#cfd8e5] hover:shadow-md">
                  ...
                  <p className="mt-2 text-sm font-semibold text-[#94a3b8]">#{article.code || "ART"}</p>

                  <div className="mt-7 border-t border-[#edf2f7] pt-5">
                    <p className="text-xs font-medium uppercase tracking-widest text-[#94a3b8]">Preis ({taxMode === "net" ? "Netto" : "Brutto"})</p>
                    <div className="mt-2 flex items-end justify-between gap-4">
                      <p className="text-[30px] font-semibold leading-none tracking-tight text-[#111827]">{formatPrice(price)} €</p>
                      <p className="pb-1 text-sm font-medium text-[#64748b]">/ {article.unit || "Stk."}</p>
                    </div>
                  </div>
                </article>
              )
              ...
              return (
                <article key={article.id} className="grid gap-4 rounded-[26px] border border-[#e5eaf0] bg-white px-5 py-4 shadow-sm md:grid-cols-[1.4fr_0.65fr_0.65fr_0.75fr_auto] md:items-center">
                  <div>
                    ...
                    <p className="mt-1 text-sm font-medium text-[#64748b]">#{article.code || "ART"} · {article.category || "Sonstiges"}</p>
                  </div>
                  <div className="text-sm font-semibold text-[#475569]">{article.tax ?? "19"}% MwSt</div>
                  <div className="text-sm font-semibold text-[#475569]">{article.unit || "Stk."}</div>
                  <div className="text-right text-xl font-semibold text-[#111827]">{formatPrice(price)} €</div>
                  <ArticleActions article={article} />
                </article>
              )
            })}
        ...
        <div className="fixed inset-0 z-[135] flex justify-end bg-black/20 px-4 py-7">
          <div className="flex h-[calc(100vh-56px)] w-full max-w-[380px] flex-col overflow-hidden rounded-[38px] bg-white shadow-[-18px_0_45px_rgba(15,23,42,0.18)]">
            <div className="flex items-start justify-between gap-4 border-b border-[#edf2f7] px-6 py-5">
              <div>
                <h2 className="text-xl font-semibold tracking-tight text-[#111827]">Artikel bearbeiten</h2>
            ...
            <div className="flex-1 space-y-4 overflow-auto px-6 py-5">
              <div className="grid grid-cols-[1fr_0.78fr] gap-3">
                <Input placeholder="Bezeichnung *" value={editForm.name} onChange={(event) => updateEditForm("name", event.target.value)} />
                <Input placeholder="Artikel-Nr." value={editForm.code} onChange={(event) => updateEditForm("code", event.target.value)} />
              </div>

              <Input placeholder="Kategorie" value={editForm.category} onChange={(event) => updateEditForm("category", event.target.value)} />

              <div className="border-t border-[#edf2f7] pt-5">
                <h3 className="mb-4 text-sm font-semibold text-[#111827]">Preise & Steuer</h3>

                <div className="grid grid-cols-[1fr_0.78fr] gap-3">
                  <Input placeholder="Preis netto *" value={editForm.price} onChange={(event) => updateEditForm("price", event.target.value)} />
                  <Input placeholder="Einheit" value={editForm.unit} onChange={(event) => updateEditForm("unit", event.target.value)} />
                </div>

                <div className="mt-4">
                  <TaxRateControl value={editForm.tax} onChange={(value) => updateEditForm("tax", value)} />
                </div>
              </div>
              ...
              <div className="border-t border-[#edf2f7] pt-5">
                ...
                <p className="mt-2 text-xs font-medium text-[#94a3b8]">
                  Wird auf der Rechnung unter dem Titel angezeigt.
                </p>
              </div>
            </div>
        ...
        <div className="fixed inset-0 z-[130] flex items-center justify-center bg-black/35 px-4">
          <div className="w-full max-w-5xl rounded-[30px] bg-white p-6 shadow-2xl">
            <div className="flex items-start justify-between gap-4">
              <div>
                <h2 className="text-2xl font-semibold text-[#111827]">Artikel importieren</h2>
                <p className="mt-1 text-sm font-medium text-[#64748b]">Erkennen, prüfen, korrigieren und dauerhaft speichern.</p>
              </div>
              <button onClick={() => setImportOpen(false)} className="flex h-10 w-10 items-center justify-center rounded-full bg-[#edf1f6] text-slate-700 hover:bg-[#e4eaf2]" aria-label="Schließen">
                <X className="h-5 w-5" />
              </button>
            </div>
            ...
            {importStep === "upload" ? (
              <div className="mt-6 rounded-[24px] border border-dashed border-[#cfd8e5] bg-[#f8fafc] p-8 text-center">
                ...
                <h3 className="mt-5 text-xl font-semibold text-[#111827]">CSV oder TXT hochladen</h3>
                <p className="mx-auto mt-2 max-w-xl text-sm font-medium text-[#64748b]">Aktuell werden CSV und TXT echt ausgelesen.</p>
                <label className="mt-6 inline-flex cursor-pointer rounded-full bg-black px-6 py-3 text-sm font-semibold text-white hover:bg-slate-800">
                  Datei auswählen
              ...
              <div className="mt-6">
                <div className="mb-4 flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
                  <div>
                    <p className="text-sm font-medium text-[#111827]">Erkannte Datei: {fileName}</p>
                    <p className="mt-1 text-sm font-medium text-[#64748b]">Bitte prüfen und bei Bedarf direkt korrigieren.</p>
                  </div>
                  <button onClick={() => setImportStep("upload")} className="rounded-full bg-[#eef2f7] px-4 py-2 text-sm font-medium text-[#334155]">Andere Datei</button>
                </div>
                  ...
                  <table className="w-full text-sm">
                    <thead className="bg-[#f8fafc] text-left text-xs font-medium uppercase tracking-widest text-[#64748b]">
                      <tr>
                        ...
                        <th className="px-4 py-3">Preis netto</th>
                      ...
                      {importRows.map((row) => (
                        <tr key={row.id}>
                          ...
                          <td className="px-3 py-3 text-right">
                            <button onClick={() => removeImportRow(row.id)} className="inline-flex h-9 w-9 items-center justify-center rounded-full bg-red-50 text-red-600 hover:bg-red-100" aria-label="Zeile löschen">
                              <Trash2 className="h-4 w-4" />
                            </button>
                          </td>
                        </tr>
                ...
                <div className="mt-6 flex justify-end gap-3">
                  <button onClick={() => setImportStep("upload")} className="rounded-full bg-[#eef2f7] px-5 py-2.5 text-sm font-medium text-[#334155]">Zurück</button>
                  <button onClick={importArticles} className="inline-flex items-center gap-2 rounded-full bg-black px-6 py-2.5 text-sm font-semibold text-[var(--brand-lime)]">
                    <Check className="h-4 w-4" />
                    Artikel speichern
                  </button>
                </div>
        ...
        <div className="fixed inset-0 z-[120] flex justify-end bg-black/35">
          <div className="h-full w-full max-w-[430px] border-l border-[#dfe6ee] bg-[#f8f9fb] shadow-[-10px_0_35px_rgba(0,0,0,0.16)]">
            <div className="flex items-start justify-between border-b border-[#e6ebf1] px-6 py-5">
              <div>
                <h2 className="text-xl font-semibold text-[#1b2333]">Neuer Artikel</h2>
                <p className="mt-1 text-sm font-medium text-[#7b8799]">Produkt oder Leistung speichern</p>
              </div>
              <button onClick={() => setPanelOpen(false)} className="flex h-10 w-10 items-center justify-center rounded-full bg-[#edf1f6] text-slate-700 hover:bg-[#e4eaf2]" aria-label="Schließen">
                <X className="h-5 w-5" />
              </button>
            </div>

            <div className="h-[calc(100%-148px)] space-y-4 overflow-auto px-6 py-5">
              <Input placeholder="Bezeichnung *" value={form.name} onChange={(event) => updateForm("name", event.target.value)} />
              <Input placeholder="Artikel-Nr." value={form.code} onChange={(event) => updateForm("code", event.target.value)} />
              <Input placeholder="Kategorie" value={form.category} onChange={(event) => updateForm("category", event.target.value)} />
              <div className="grid grid-cols-2 gap-3">
                <Input placeholder="Preis netto *" value={form.price} onChange={(event) => updateForm("price", event.target.value)} />
                <Input placeholder="Einheit" value={form.unit} onChange={(event) => updateForm("unit", event.target.value)} />
              </div>
              <TaxRateControl value={form.tax} onChange={(value) => updateForm("tax", value)} />
              <textarea value={form.description} onChange={(event) => updateForm("description", event.target.value)} className="w-full rounded-[18px] border border-[#dfe6ee] bg-[#eef2f7] px-4 py-3 text-sm font-medium text-[#334155] outline-none focus:ring-2 focus:ring-slate-900" rows={5} placeholder="Beschreibung" />
            </div>
